...
st.title("Управление Транспортными Предприятиями (БД)")
...
with st.form("add_enterprise_form"):
    ...
    if submitted:
        if enterprise_name.strip() and description.strip():
            add_to_db(enterprise_name, description)  # Добавляем в базу данных
            st.session_state.enterprises = load_data_from_db()  # Обновляем данные в сессии
            st.success("Предприятие успешно добавлено.")
        else:
